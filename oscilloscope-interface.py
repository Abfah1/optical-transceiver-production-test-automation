...
class SimulatedOscilloscope:
    """
    A simulated oscilloscope for testing without physical hardware.
    Generates a realistic sine wave with noise using numpy.
    """

    idn: str = "SIM::RIGOL_DS1202"
    _volts_per_div: float = 1.0
    _time_per_div: float = 200e-6

    def connect(self) -> None:
        print(f"[SIM] Connected: {self.idn}")
    ...
    def set_timebase(self, time_per_div: float) -> None:
        self._time_per_div = time_per_div

    def set_voltage_scale(self, channel: int, volts_per_div: float) -> None:
        self._volts_per_div = volts_per_div
    ...
    def get_waveform(self, channel: int) -> Tuple[np.ndarray, np.ndarray]:
        """
        Return a simulated sine wave with noise.
        Amplitude scales with the current voltage scale setting.

        :param channel: Ignored in simulation (kept for API compatibility).
        :return: Tuple of numpy arrays (time, voltage).
        """
        n = 1000
        freq = 1000.0  # 1 kHz sine wave
        # Show ~2 full cycles across the time window
        total_time = max(2.0 / freq, self._time_per_div * 10)
        t = np.linspace(0, total_time, n)

        # Amplitude = 3 divisions peak, scaled by volts/div
        amplitude = self._volts_per_div * 3.0
        noise_std = amplitude * 0.04  # ~4% noise
        noise = np.random.normal(0, noise_std, n)
        voltage = amplitude * np.sin(2 * np.pi * freq * t) + noise

        print(f"[SIM] Waveform generated: amplitude={amplitude:.3f} V, "
              f"freq={freq:.0f} Hz, points={n}")
        return t, voltage
...
class MainWindow(QMainWindow):
    ...
    def _setup_ui(self) -> None:
        ...
        self.sim_checkbox = QCheckBox("Simulation Mode (no hardware)")
        self.sim_checkbox.setToolTip(
            "When checked, a simulated oscilloscope is used — no physical device needed."
        )
        self.sim_checkbox.toggled.connect(self._on_sim_mode_toggled)
        ...
        # ── Top connection row (sim checkbox + resource input) ────────────────
        connection_layout = QHBoxLayout()
        connection_layout.addWidget(self.sim_checkbox)
        connection_layout.addSpacing(20)
        connection_layout.addWidget(self.resource_label)
        connection_layout.addWidget(self.resource_input)
    ...
    def _on_sim_mode_toggled(self, checked: bool) -> None:
        """Gray out the resource name field when simulation mode is active."""
        self.resource_input.setEnabled(not checked)
        self.resource_label.setEnabled(not checked)

    def connect_oscilloscope(self) -> None:
        """
        Connect to the oscilloscope (real or simulated) based on the checkbox state.
        """
        if self.sim_checkbox.isChecked():
            ...
            try:
                ...
                self.status_label.setText(
                    f"Status: Connected (Simulation) — {SimulatedOscilloscope.idn}"
                )
            ...
            try:
                ...
                QMessageBox.critical(
                    ...
                    "Connection Error",
                    f"Connection failed:\n{e}\n\n"
                    "Tip: Enable \"Simulation Mode (no hardware)\" to run without a physical device."
                )
    ...
    def apply_settings(self) -> None:
        ...
        if not self.oscilloscope:
            # Silently skip if called before connection (e.g. from valueChanged signals)
            return
    ...
    def select_waveform_color(self):
        ...
        if color.isValid():
            ...
            if self.time is not None and self.voltage is not None:
                self.plot_waveform(self.time, self.voltage)
